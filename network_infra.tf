...
resource "aws_vpc_dhcp_options" "workspaces_vpc" {
  ...
  domain_name_servers = slice(var.ad_dns_ips, 0, 2) # AD DNS IPs in EVS VPC (first two IPs only from the list)
  ntp_servers         = slice(var.ad_dns_ips, 0, 2) # Optional: Use AD servers for NTP (first two IPs only from the list)
  tags = {
...
# Import existing TGW attachment from evs-vpc
import {
  id = "tgw-attach-0bf0af7503aaf5f94"
  to = aws_ec2_transit_gateway_vpc_attachment.evs-vpc
}
...
moved {
  from = aws_ec2_transit_gateway_vpc_attachment.workspaces[0]
  to   = aws_ec2_transit_gateway_vpc_attachment.workspaces-vpc
}
# Create TGW VPC attachment for workspaces-VPC
resource "aws_ec2_transit_gateway_vpc_attachment" "workspaces-vpc" {
